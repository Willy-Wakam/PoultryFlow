...
Cross-cutting security owns token validation, fail-closed routing, allowlisted `poultryflow-api` client-role mapping, and safe 401/403 responses. The frontend's protected-request boundary refreshes tokens proactively, attaches bearer credentials only to same-origin `/api/**` requests, never retries writes automatically, and expires the online session after a backend 401. Authorization policies remain close to controller and use-case boundaries through method security. Frontend role guards are a user-experience aid only; the backend remains authoritative.
